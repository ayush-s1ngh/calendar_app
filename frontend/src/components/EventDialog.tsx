...
const EventDialog = ({ open, onClose, event }: EventDialogProps) => {
  ...
  // Debug information
  console.log("=== EVENT DIALOG DEBUG ===");
  ...
  const [formData, setFormData] = useState({
    ...
    end_datetime: normalizeDate(new Date(Date.now() + 60 * 60 * 1000)), // 1 hour from now
    is_all_day: false,
    color: 'blue'
  });
  ...
  useEffect(() => {
    if (isEditMode && fullEvent) {
      // Use full event data from the API when available
      console.log("Setting form data from full event:", fullEvent);
      setFormData({
        title: fullEvent.title || '',
        description: fullEvent.description || '',
        start_datetime: normalizeDate(new Date(fullEvent.start_datetime)),
        end_datetime: normalizeDate(new Date(fullEvent.end_datetime)),
        is_all_day: fullEvent.is_all_day || false,
        color: fullEvent.color || 'blue'
      });
    } else if (event) {
      // Initial setup from calendar event object
      console.log("Setting form data from calendar event:", event);
      setFormData({
        title: event.title || '',
      ...
      console.log("Setting form data for new event");
      setFormData({
        title: '',
        description: '',
        start_datetime: normalizeDate(new Date()),
  ...
  const handleDateChange = (field: 'start_datetime' | 'end_datetime', newDate: Date | null) => {
    console.log(`Changing ${field} to:`, newDate);

    if (newDate) {
      // Make sure the date is properly normalized
      ...
      <form onSubmit={handleSubmit}>
        ...
        <DialogContent>
          <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2, mt: 1 }}>
            ...
            {isEditMode && event.id && <ReminderSection eventId={event.id} />}
          </Box>
        </DialogContent>
        <DialogActions>
          ...
          <Button
            ...
            variant="contained"
            disabled={isLoading}
            startIcon={(createMutation.isPending || updateMutation.isPending) && <CircularProgress size={20} />}
          >
