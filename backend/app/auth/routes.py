...
@jwt_required()
@rate_limit(limit=3, window=300)  # 3 resend attempts per 5 minutes
...
@rate_limit(limit=3, window=300)  # 3 password reset requests per 5 minutes
...
@rate_limit(limit=5, window=300)  # 5 password reset attempts per 5 minutes
...
def google_callback():
    ...
    try:
        ...
        # Generate auth tokens for our app
        tokens = generate_auth_tokens(user)
        
        logger.info(f"Google OAuth login successful for user: {user.username}")
        
        # Redirect to frontend with tokens (or return JSON if called via AJAX)
